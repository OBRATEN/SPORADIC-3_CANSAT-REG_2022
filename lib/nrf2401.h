...
#define nRF_CSN_LOW() PORTB &= ~(1 << CSN);
#define nRF_CSN_HIGH() PORTB |= (1 << CSN);
#define nRF_CE_LOW() PORTB &= ~(1 << CE);
#define nRF_CE_HIGH() PORTB |= (1 << CE);

void nRF_init(void);
uint8_t nRF_writeBuf(uint8_t addr, uint8_t *buf, uint8_t count);
uint8_t nRF_writeReg(uint8_t addr, uint8_t val);
uint8_t nRF_writeRegBuf(uint8_t addr, uint8_t *data, uint8_t count);
